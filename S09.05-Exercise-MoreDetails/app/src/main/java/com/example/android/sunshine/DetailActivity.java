...
//      COMPLETED (21) Implement LoaderManager.LoaderCallbacks<Cursor>
    ...
    private final int COL_DATE_INDEX = 0;
    private final int COL_DESCRIPTION_INDEX = 1;
    private final int COL_MAX_INDEX = 2;
    private final int COL_MIN_INDEX = 3;
    private final int COL_HUMIDITY_INDEX = 4;
    private final int COL_WIND_INDEX = 5;
    private final int COL_PRESSURE_INDEX = 6;
    private final int COL_DEGREES_INDEX = 7;

    //  COMPLETED (20) Create a constant int to identify our loader used in DetailActivity
    private final int WEATHER_LOADER_ID = 200;
...
//      COMPLETED (16) Use getData to get a reference to the URI passed with this Activity's Intent
        if (intentThatStartedThisActivity != null) {
            ...
            if (mUri == null) {
                throw new NullPointerException("Uri needed");
            }
        }
//      COMPLETED (35) Initialize the loader for DetailActivity
        getSupportLoaderManager()
                .initLoader(WEATHER_LOADER_ID, null, this);
    }
...
//      COMPLETED (26) Display a readable data string
            long dateInMillis = data.getLong(COL_DATE_INDEX);
            String formattedDate = SunshineDateUtils.getFriendlyDateString(this, dateInMillis, true);
            mDateTextView.setText(formattedDate);
//      COMPLETED (27) Display the weather description (using SunshineWeatherUtils)
            String weatherCondition = SunshineWeatherUtils.getStringForWeatherCondition(this, data.getInt(COL_DESCRIPTION_INDEX));
            mDescriptionTextView.setText(weatherCondition);
//      COMPLETED (28) Display the high temperature
            mHighTempTextView.setText(String.valueOf(data.getFloat(COL_MAX_INDEX)));
//      COMPLETED (29) Display the low temperature
            mLowTempTextView.setText(String.valueOf(data.getFloat(COL_MIN_INDEX)));
//      COMPLETED (30) Display the humidity
            mHumidityTextView.setText(String.valueOf(data.getFloat(COL_HUMIDITY_INDEX)));
//      COMPLETED (31) Display the wind speed and direction
            String windFormatted = SunshineWeatherUtils.getFormattedWind(
                    this,
                    data.getFloat(COL_WIND_INDEX),
                    data.getFloat(COL_DEGREES_INDEX));
            mWindTextView.setText(windFormatted);
//      COMPLETED (32) Display the pressure
            mPressureTextView.setText(String.valueOf(data.getFloat(COL_PRESSURE_INDEX)));
//      COMPLETED (33) Store a forecast summary in mForecastSummary
            mForecastSummary = String.format("%s, %s", formattedDate, weatherCondition);
