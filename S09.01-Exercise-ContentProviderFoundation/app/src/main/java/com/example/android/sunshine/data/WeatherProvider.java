...
//      COMPLETED (9) Handle queries on both the weather and weather with date URI
        ...
        switch (match){
            case CODE_WEATHER:
                queryResult = database.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder);
                break;
            case CODE_WEATHER_WITH_DATE:
                int date = Integer.parseInt(uri.getLastPathSegment());
                queryResult = database.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, WeatherContract.WeatherEntry.COLUMN_DATE + "=" + date, null, null, null, sortOrder);
                break;
            default:
